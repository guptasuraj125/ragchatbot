...
function createMessage(role, text = '') {
    ...
    image.alt = role === 'user' ? 'User avatar' : 'AI avatar';
    image.src = role === 'user'
...
pdfInput.addEventListener('change', async () => {
    ...
    try {
        ...
        uploadStatus.textContent = `${data.document_name} uploaded`;
        createMessage('assistant', `${data.document_name} uploaded successfully. You can start chatting now.`);
        saveMessages();
    } catch (error) {
...
form.addEventListener('submit', async (event) => {
    ...
    try {
        ...
        await typeWords(assistantBubble, sanitizeResponseText(data.answer));
    } catch (error) {
